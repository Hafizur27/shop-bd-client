...
const LogIn = () => {
  ...
  const onSubmit = (data) => {
    axios.post("http://localhost:5000/login", data).then((res) => {
      if (res.data.modifiedCount !== 0) {
        ...
        toast.success("well come to shop_bd", {position: 'top-right'})
        reset();
        navigate("/products");
      }
    });
  ...
  return (
    <div className="flex items-center">
      <div className="card bg-[#FFF] border border-[#EEE] rounded-2xl shadow-xl w-full sm:max-w-[444px] mx-auto pb-4">
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="card-body flex flex-col gap-5 p-4 md:p-8 lg:p-12"
        >
          <div className="flex flex-col gap-5 items-center md:items-start">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-[#4E5D78] text-2xl md:text-3xl font-bold">SHOP_BD</h2>
            </div>
            <div>
              <h3 className="text-[#404040] text-xl md:text-2xl font-semibold">
                Log In to join with Shop_Bd
              </h3>
            </div>
          </div>
